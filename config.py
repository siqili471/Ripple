"""
config.py — Central configuration for the Ripple experiment.

Contrastive decoding formula:
    logits_final = logits_neutral + α * (logits_neutral - logits_penalized)

    α < 0  →  discouragement (push toward penalized, away from neutral)
    α = 0  →  neutral baseline (pure neutral prompt)
    α > 0  →  encouragement  (push away from penalized, toward neutral)
"""
